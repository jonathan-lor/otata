...
on macOS, nothing where no service manager is wired up. It speaks the
manager's own language and nothing else. What an installed unit means for
this root, how one is brought to a bound server, and what to say when it
cannot be, is App's to decide, once, in autostart.go.
...
type supervisor interface {
	...
	// Loaded reports whether the manager currently manages the unit, whoever
	// it belongs to.
	Loaded() bool
	// Disabled reports whether the user switched the unit off at the
	// manager's own level, where loading it fails with no useful word.
	Disabled() bool
	// Enable clears that switch. Only an explicit `autostart on` may.
	Enable()
	// Install writes the unit's definition, replacing any. It loads nothing.
